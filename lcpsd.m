function [Pxx, f, C] = lcpsd(xa, xb, windowfcn, fmin, fmax, Jdes, Kdes, Kmin, fs, xi)
...
%   The parameters are:
%
%     xa,xb:      time series to be transformed
...
%   revised by Xiaohui SHi <shixiaohui@hust.edu.cn> 2017-05-13
...
for jj=1:length(f)
  ...
  data_a = bsxfun(@minus, data_a, mean(data_a));                          % (4)
  data_b = bsxfun(@minus, data_b, mean(data_b));
  
  % Compute the discrete Fourier transform
  window = windowfcn(L(jj));                                        % (5)
  sinusoid = exp(-2i*pi * (0:L(jj)-1)' * m(jj)/L(jj));              % (6)  
  data_a = bsxfun(@times, data_a, sinusoid .* window);                  % (5,6)
  data_b = bsxfun(@times, data_b, sinusoid .* window);
  
  % Average the squared magnitudes
%     Pxx(jj) = mean(abs(sum(data_a)).^2);                                % (8)
%   Pxx(jj) = mean((sum(data_a)).*(sum(data_b)));  wrong                              % (8)
  Pxx(jj) = mean((sum(data_a)).*conj((sum(data_b))));
